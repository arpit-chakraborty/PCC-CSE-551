...
**Question:** Write a PL/SQL block of code that inserts a record representing the opening balance entry in the TRANS_MSTR20231056 table each time an account is opened. The insertion should only occur if the ACCT_NO is not already present in the TRANS_MSTR20231056 table (indicating number of initial deposit has been made). If the ACCT_NO exists

```sql
EDIT PLSQL20231056;

CREATE OR REPLACE FUNCTION F_CHKACCTNO(vACCT_NO IN VARCHAR) RETURN NUMBER IS
    dummyAcctNo VARCHAR(10);
BEGIN
